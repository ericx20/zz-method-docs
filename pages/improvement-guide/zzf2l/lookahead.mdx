...
2. Set the left bar on the top to '3x3x3', and then the right bar to 'Custom'. Then in the settings set the Scramble options: clear the 'OriE' section, and disable the 'PermE-DL', 'PermE-DF', 'PermE-DR' and 'PermE-DB' options. This will generate scrambles with EOCross already solved.
...
3. Solve pair A. While solving this pair, spot the pieces of another F2L pair (pair B).
4. Repeat step 3 until all pairs are solved.

(I assume that by now you would have figured out that you don't need to focus on another pair while you solve your last slot. Instead, we can focus on the OCLL you're going to get, but this is more advanced and we will talk about this later)

You could have noticed that this drill is exactly like the previous one, just with steps 3 and 4 merged into a single step. So instead of spotting your pieces of Pair B beforehand, you need to spot them on the fly as you solve Pair A. This is a big spike in difficulty, so at first you want to make it simple by slowing down your turning so that your mind can keep track of pieces for pair B, even if that means 1 turn per second (just make sure that your turning is fluid, otherwise you're just doing metronome practice, you might have heard of it before but it's not effective)
...
In this insert, we can see that FR and DFR are kept together. Same with UL and UBL. You may also notice that the UFL corner is twisted counterclockwise, and so on. Having an idea of how this insert moves pieces around is essential to knowing where pieces will end up.
...
We can also use a modification of our very first drill mentioned in here, you spot Pair A and the pieces for Pair B, but then you close your eyes and sol ve these 2 pairs blindfolded as fast as possible. This will let you put the before mentioned process of seeing how pieces move around in practice.
