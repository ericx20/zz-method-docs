...
[OO ZZ F2L doc](https://docs.google.com/spreadsheets/d/172R8dkejxLXw9rj9E0B0plvYJtKnYnBVh_m3caNbFrM)

This is the resource you would want to use, it covers every single possible pair you can get in ZZ, as well as alternative solutions to different combinations of **open slots**. A fantastic resource, its all you need to optimize your pair solutions.
...
Tip: it's good to know all of them in the long run, as they can help in OLS or forcing better OCLLs.
...
But with FR slot open, there's no need for that. This mean the case could now be solved with just `R U2 R2 U' R`:
...
Then `R U' R'` to insert the edge to the slot it belongs, and `D` to undo the first D move.
...
So the only cases left are the ones with both pieces in the F2L slots, what about those? Bar a few good cases, it's not recommended to learn them. Why? Because when these cases apear in a solve, there's a very high chance that there will be an another F2L pair on the cube thats more efficient and faster to solve, so we could just choose that pair instead. This is called [Pair Choice](/improvement-guide/zzf2l/pair-choice).
