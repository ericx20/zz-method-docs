...
Looking at the BL edge alone, we cannot deduce it because we can only a single (orange) sticker. But we can use the process of elimination on all edges that have orange on them:
...
For corners, you can also use process of elimination, but it's harder and less consistent. You may need to also deduce 'medium' corners. However, if you know exactly what corner you're trying to identify (so a red-green-white for example), you can identify it by just 1 sticker.
...
Because EO is solved, there is a very special rule you can use for the UB edge:

<Callout>
  If the UB edge doesn't have a U sticker on top, it can't be a LL edge. **It's
  an F2L edge**.
</Callout>
...
Since EOCross is solved, all bottom edges are solved. So during F2L, there are two types of edges: F2L edges (belonging to an F2L pair) and LL edges. LL edges belong to the top layer, so they'll always have a U sticker (a sticker that matches the colour of the U center).

Typically, you need to see both stickers of an edge to tell if it's a LL edge. However, there is a special rule for the UB edge! Its only visible sticker is on the top side, which happens to be part of the ['purple orbit'](/tutorial/eo#orbits). Because EO is solved, U stickers will always be on the purple orbit. This means: if the UB edge is an LL edge, you will see a U sticker on its top side. _If you don't see a U sticker there, the UB edge is an F2L edge._

</details>

You can use this rule together with process of elimination. Here's an example:
...
But the UB Edge Rule says: the UB edge doesn't have a U sticker on top (it doesn't have yellow on top, it's just green), therefore it's an F2L edge. This is perfect, because it eliminates the green-yellow LL edge which we weren't sure of.

- ❌green-yellow (LL edge): nope because of UB Edge Rule. We didn't see yellow.
- ❌green-red (F2L edge): it's an 'easy' piece, we see it at UF.
